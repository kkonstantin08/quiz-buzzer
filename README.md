...
`APP_PUBLIC_URL` — публичный origin frontend без завершающего `/`; ссылка имеет вид `https://qbuz.ru/reset-password#token=...`. Frontend сразу удаляет fragment из адресной строки и хранит токен только в состоянии страницы. TTL принимает целое число от 1 до 30 минут, по умолчанию — 30.
