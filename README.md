...
Без `TRUST_PROXY` backend игнорирует клиентский `X-Forwarded-For`. Docker Compose запускает nginx и backend в выделенной внутренней сети: backend доверяет только фиксированному IP nginx (`172.30.0.2`). Nginx принимает `CF-Connecting-IP` только от фиксированного контейнера `cloudflared` (`172.30.0.3`), затем перезаписывает `X-Real-IP` и `X-Forwarded-For`; произвольный заголовок клиента не передаётся дальше. Для API, Socket.IO и uploads nginx также передаёт `Host` и `X-Forwarded-Proto`.
